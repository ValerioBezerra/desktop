...
procedure TfrmGER004.btnSalvarClick(Sender: TObject);
begin
  cdsPadrao.Post;
...
procedure TfrmGER004.dbneGER_GERCID_BAIExit(Sender: TObject);
begin
  ...
  if (cmKingGeral.cdsConsulta.IsEmpty) then
    edGER_NOME_CID.Text := 'Estado não cadastrado'
  else
    edGER_NOME_CID.Text := cmKingGeral.cdsConsulta.FieldByName('GER_NOME_CID').AsString;
end;

procedure TfrmGER004.dbneGER_GERCID_BAIKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TfrmGER004.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfrmGER004.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TfrmGER004.sbGER003Click(Sender: TObject);
begin
  inherited;
  try
