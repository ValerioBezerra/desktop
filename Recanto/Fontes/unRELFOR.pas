...
procedure TfrmRELFOR.Button1Click(Sender: TObject);
begin
   prepararSQL;
...
procedure TfrmRELFOR.Button2Click(Sender: TObject);
begin
   Close;
end;

procedure TfrmRELFOR.FormCreate(Sender: TObject);
begin
    cdspadrao := DataModule1.cdsFORNECEDOR;
end;

procedure TfrmRELFOR.prepararSQL;
begin
    cdspadrao.Close;
    cdspadrao.CommandText := 'select * from fornecedor';
    cdspadrao.Open;

    if rdgOrdem.ItemIndex = 0 then
       cdspadrao.CommandText := cdspadrao.CommandText + 'order by codigo_for'
    else
      cdspadrao.CommandText := cdspadrao.CommandText + 'order by nome_for';
