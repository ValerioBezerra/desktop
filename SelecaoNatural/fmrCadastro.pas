...
begin
        ...
        edCódigo.Text := dm.DataModule2.cdsPRO.FieldByName('COD_PRODUTO').AsString;
...
begin
    ...
    begin
      ...
      showmessage('Campos não preenchidos' + #13 + TestarCampos);
end;

procedure TfrmCadastro.Image3Click(Sender: TObject);
begin
       If  MessageDlg('Você tem certeza que deseja descartar os dados?',mtConfirmation,[mbyes,mbno],0)=mryes
   then
      begin
        dm.DataModule2.cdsPRO.Close;
        edCódigo.Clear;
...
begin
    ...
    if dbedDESCRICAO.Text = '' then
        retorno := 'Descrição do item ';
    if dbedPRECO.Text = '' then
         retorno := retorno + #13 + 'Preço do Item';
