...
procedure TfrmRELPREVPAG.Button1Click(Sender: TObject);
begin
   prepararSQL;
...
procedure TfrmRELPREVPAG.Button2Click(Sender: TObject);
begin
   Close;
end;

procedure TfrmRELPREVPAG.FormCreate(Sender: TObject);
begin
    cdspadrao := DataModule1.cdsPAGTIT;
end;

procedure TfrmRELPREVPAG.ppDetailBand1AfterPrint(Sender: TObject);
begin
    varTotalAnalitico.Value := varTotalAnalitico.Value + cdspadrao.FieldByName('VALORABERTO').AsCurrency;
end;

procedure TfrmRELPREVPAG.ppDetailBand1BeforePrint(Sender: TObject);
begin
   if cdspadrao.FieldByName('ORIGEM_PAGTIT').AsInteger = 0 then
...
procedure TfrmRELPREVPAG.ppDetailBand2AfterPrint(Sender: TObject);
begin
      varTotal.Value := varTotal.Value + cdspadrao.FieldByName('VALORABERTO').AsCurrency;
end;

procedure TfrmRELPREVPAG.ppFooterBand1BeforePrint(Sender: TObject);
begin
     varDataInicialAnalitico.Value := DateTimePicker1.Date;
   varDataFinalAnalitico.Value := DateTimePicker2.Date;
end;

procedure TfrmRELPREVPAG.ppFooterBand2BeforePrint(Sender: TObject);
begin
   varDatainicial.Value := DateTimePicker1.Date;
   varDataFinal.Value := DateTimePicker2.Date;
end;

procedure TfrmRELPREVPAG.prepararSQL;
var
where :String;
begin
    varTotal.Value := 0;
    cdspadrao.Close;
    cdspadrao.CommandText := 'select CODFOR_PAGTIT,SEQ_PAGTIT, ORIGEM_PAGTIT,DATAEMISSAO_PAGTTIT,NOME_FOR,' +
                             ' DATAVENC_PAGTIT,VALORTOTAL_PAGTIT,VALORPAGO_PAGTIT,(VALORTOTAL_PAGTIT - VALORPAGO_PAGTIT) AS VALORABERTO' +
                             ' from PAGTIT LEFT OUTER JOIN FORNECEDOR ON CODIGO_FOR = CODFOR_PAGTIT where 1=1 ';
      if DateTimePicker1.Date > DateTimePicker2.Date then
          ...
          if rdgOpcao.ItemIndex = 0 then
              where := where + 'and DATAEMISSAO_PAGTIT between ' +  QuotedStr(StringReplace((DatetoStr(DateTimePicker1.Date)),'/','.', [rfReplaceAll, rfIgnoreCase])) + 'and ' + QuotedStr(StringReplace((DatetoStr(DateTimePicker2.Date)),'/','.', [rfReplaceAll, rfIgnoreCase]))
          else
